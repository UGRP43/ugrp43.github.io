...
(function strip(){
  ...
  document.querySelectorAll("svg#strip, svg.strip").forEach(svg=>{
    const keys=(svg.dataset.rows||"132_base,132_hamlet,315_base,315_hamlet")
      .split(",").map(k=>k.trim()).filter(k=>Object.hasOwn(D,k));
    const H=T+B+RH*keys.length, iw=W-L-R, rh=RH, x=v=>L+v*iw;
